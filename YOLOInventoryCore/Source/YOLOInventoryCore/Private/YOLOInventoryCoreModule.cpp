...
DEFINE_LOG_CATEGORY_STATIC(LogYOLOInventoryCoreModule, Log, All);

namespace YOLOInventoryCoreModulePrivate
{
...
public:
	virtual void StartupModule() override
	{
		DebugConsoleCommand = MakeUnique<FAutoConsoleCommandWithWorldAndArgs>(
			...
			FConsoleCommandWithWorldAndArgsDelegate::CreateRaw(this, &FYOLOInventoryCoreModule::HandleDebugConsoleCommand));
		...
		DebugPipelineConsoleCommand = MakeUnique<FAutoConsoleCommandWithWorldAndArgs>(
			...
			FConsoleCommandWithWorldAndArgsDelegate::CreateRaw(this, &FYOLOInventoryCoreModule::HandleDebugPipelineConsoleCommand));
		...
		DebugScreenConsoleCommand = MakeUnique<FAutoConsoleCommandWithWorldAndArgs>(
			...
			FConsoleCommandWithWorldAndArgsDelegate::CreateRaw(this, &FYOLOInventoryCoreModule::HandleDebugScreenConsoleCommand));
		...
		DebugLogConsoleCommand = MakeUnique<FAutoConsoleCommandWithWorldAndArgs>(
			...
			FConsoleCommandWithWorldAndArgsDelegate::CreateRaw(this, &FYOLOInventoryCoreModule::HandleDebugLogConsoleCommand));
		...
		DebugForceConsoleCommand = MakeUnique<FAutoConsoleCommandWithWorldAndArgs>(
			...
			FConsoleCommandWithWorldAndArgsDelegate::CreateRaw(this, &FYOLOInventoryCoreModule::HandleDebugForceConsoleCommand));
		...
		DebugChannelConsoleCommand = MakeUnique<FAutoConsoleCommandWithWorldAndArgs>(
			...
			FConsoleCommandWithWorldAndArgsDelegate::CreateRaw(this, &FYOLOInventoryCoreModule::HandleDebugChannelConsoleCommand));
		...
		DebugHistoryConsoleCommand = MakeUnique<FAutoConsoleCommandWithWorldAndArgs>(
			...
			FConsoleCommandWithWorldAndArgsDelegate::CreateRaw(this, &FYOLOInventoryCoreModule::HandleDebugHistoryConsoleCommand));
		...
		DebugStatusConsoleCommand = MakeUnique<FAutoConsoleCommandWithWorldAndArgs>(
			...
			FConsoleCommandWithWorldAndArgsDelegate::CreateRaw(this, &FYOLOInventoryCoreModule::HandleDebugStatusConsoleCommand));
		...
		DebugProfileConsoleCommand = MakeUnique<FAutoConsoleCommandWithWorldAndArgs>(
			...
			FConsoleCommandWithWorldAndArgsDelegate::CreateRaw(this, &FYOLOInventoryCoreModule::HandleDebugProfileConsoleCommand));
	}
...
private:
	void HandleDebugConsoleCommand(const TArray<FString>& Args, UWorld* /*World*/)
	{
		...
		if (Args.Num() > 0)
		{
			YOLOInventoryCoreModulePrivate::ParseToggleArg(Args[0], UYOLOInventorySettings::Get().bShowDebug, bEnable);
		}
		...
		UE_LOG(LogYOLOInventoryCoreModule, Display, TEXT("YOLOInventory debug %s"), bEnable ? TEXT("ENABLED") : TEXT("DISABLED"));
	}
	...
	void HandleDebugPipelineConsoleCommand(const TArray<FString>& Args, UWorld* /*World*/)
	{
		...
		if (Args.Num() > 0)
		{
			YOLOInventoryCoreModulePrivate::ParseToggleArg(Args[0], Settings.bEnableDebugPipeline, bEnable);
		}
		Settings.bEnableDebugPipeline = bEnable;
		Settings.SaveConfig();
		UE_LOG(LogYOLOInventoryCoreModule, Display, TEXT("YOLOInventory debug pipeline %s"), bEnable ? TEXT("ENABLED") : TEXT("DISABLED"));
	}
	...
	void HandleDebugScreenConsoleCommand(const TArray<FString>& Args, UWorld* /*World*/)
	{
		...
		if (Args.Num() > 0)
		{
			YOLOInventoryCoreModulePrivate::ParseToggleArg(Args[0], Settings.bDebugOutputToScreen, bEnable);
		}
		Settings.bDebugOutputToScreen = bEnable;
		Settings.SaveConfig();
		UE_LOG(LogYOLOInventoryCoreModule, Display, TEXT("YOLOInventory debug screen output %s"), bEnable ? TEXT("ENABLED") : TEXT("DISABLED"));
	}
	...
	void HandleDebugLogConsoleCommand(const TArray<FString>& Args, UWorld* /*World*/)
	{
		...
		if (Args.Num() > 0)
		{
			YOLOInventoryCoreModulePrivate::ParseToggleArg(Args[0], Settings.bDebugOutputToLog, bEnable);
		}
		Settings.bDebugOutputToLog = bEnable;
		Settings.SaveConfig();
		UE_LOG(LogYOLOInventoryCoreModule, Display, TEXT("YOLOInventory debug log output %s"), bEnable ? TEXT("ENABLED") : TEXT("DISABLED"));
	}
	...
	void HandleDebugForceConsoleCommand(const TArray<FString>& Args, UWorld* /*World*/)
	{
		...
		if (Args.Num() > 0)
		{
			YOLOInventoryCoreModulePrivate::ParseToggleArg(Args[0], Settings.bAllowForcedDebugMessages, bEnable);
		}
		Settings.bAllowForcedDebugMessages = bEnable;
		Settings.SaveConfig();
		UE_LOG(LogYOLOInventoryCoreModule, Display, TEXT("YOLOInventory forced debug messages %s"), bEnable ? TEXT("ENABLED") : TEXT("DISABLED"));
	}
	...
	void HandleDebugChannelConsoleCommand(const TArray<FString>& Args, UWorld* /*World*/)
	{
		if (Args.Num() < 1)
		{
			UE_LOG(LogYOLOInventoryCoreModule, Display, TEXT("Usage: YOLOInventory.Debug.Channel <all|general|persistence|inventory|equipment|actionbar|trade|shop|grid|phase2> [0|1|toggle]"));
			return;
		}

		UYOLOInventorySettings& Settings = UYOLOInventorySettings::GetMutable();
		bool* ChannelFlag = nullptr;
		if (!YOLOInventoryCoreModulePrivate::ResolveChannelByName(Args[0], ChannelFlag, Settings))
		{
			UE_LOG(LogYOLOInventoryCoreModule, Warning, TEXT("Unknown debug channel '%s'."), *Args[0]);
			return;
		}
		...
		if (Args.Num() > 1)
		{
			...
			if (!YOLOInventoryCoreModulePrivate::ParseToggleArg(Args[1], bCurrent, bNewValue))
			{
				UE_LOG(LogYOLOInventoryCoreModule, Warning, TEXT("Invalid channel toggle value '%s'."), *Args[1]);
				return;
			}
		}
		else
		...
		Settings.SaveConfig();
		UE_LOG(LogYOLOInventoryCoreModule, Display, TEXT("YOLOInventory debug channel '%s' %s"), *Args[0], bNewValue ? TEXT("ENABLED") : TEXT("DISABLED"));
	}
	...
	void HandleDebugHistoryConsoleCommand(const TArray<FString>& Args, UWorld* /*World*/)
	{
		if (Args.Num() > 0 && Args[0].Equals(TEXT("clear"), ESearchCase::IgnoreCase))
		{
			...
			UE_LOG(LogYOLOInventoryCoreModule, Display, TEXT("YOLOInventory debug history cleared."));
			return;
		}

		TArray<FYIDebugMessageRecord> History;
		UYIDebugLibrary::GetDebugMessageHistory(History);
		UE_LOG(LogYOLOInventoryCoreModule, Display, TEXT("YOLOInventory debug history entries: %d"), History.Num());
	}
	...
	void HandleDebugStatusConsoleCommand(const TArray<FString>& /*Args*/, UWorld* /*World*/)
	{
		...
		UE_LOG(LogYOLOInventoryCoreModule, Display, TEXT("Pipeline=%s Screen=%s Log=%s Force=%s History=%s Dedupe=%s StableKeys=%s"),
		...
		UE_LOG(LogYOLOInventoryCoreModule, Display, TEXT("Durations: screen=%.2fs pinned=%.2fs duplicate_interval=%.2fs history_max=%d"),
		...
		UE_LOG(LogYOLOInventoryCoreModule, Display, TEXT("Channels: General=%d Persistence=%d Inventory=%d Equipment=%d ActionBar=%d Trade=%d Shop=%d Grid=%d Phase2=%d"),
	...
	void HandleDebugProfileConsoleCommand(const TArray<FString>& Args, UWorld* /*World*/)
	{
		if (Args.Num() < 1)
		{
			UE_LOG(LogYOLOInventoryCoreModule, Display, TEXT("Usage: YOLOInventory.Debug.Profile <off|minimal|normal|verbose>"));
			return;
		}
		...
		{
			UE_LOG(LogYOLOInventoryCoreModule, Warning, TEXT("Unknown debug profile '%s'."), *Args[0]);
			return;
		}

		Settings.SaveConfig();
		UE_LOG(LogYOLOInventoryCoreModule, Display, TEXT("YOLOInventory debug profile set to '%s'."), *Profile);
	}
