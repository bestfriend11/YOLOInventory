...
NvFaceInfo * NvStripifier::FindOtherFace(NvEdgeInfoVec &edgeInfos, int v0, int v1, NvFaceInfo *faceInfo){
	...
	if (edgeInfo) return (edgeInfo->m_face0 == faceInfo ? edgeInfo->m_face1 : edgeInfo->m_face0);
	return NULL;
}
	...
	{	
		...
		if(!AlreadyExists(faceInfo, faceInfos))
		{
			...
			if (edgeInfo01 == NULL){
				...
				{
					//gpgenericf(("BuildStripifyInfo: > 2 triangles on an edge... uncertain consequences\n"));
				}
				else
...
inline int NvStripifier::GetNextIndex(const WordVec &indices, NvFaceInfo *face){
	...
	if (fv0 != v0 && fv0 != v1){
		if ((fv1 != v0 && fv1 != v1) || (fv2 != v0 && fv2 != v1)){
			//gpgenericf(("GetNextIndex: Triangle doesn't have all of its vertices\n"));
			//gpgenericf(("GetNextIndex: Duplicate triangle probably got us derailed\n"));
		}
		return fv0;
	}
	if (fv1 != v0 && fv1 != v1){
	...
	// shouldn't get here
	//gpgenericf(("GetNextIndex: Duplicate triangle sent\n"));
	return -1;
}
