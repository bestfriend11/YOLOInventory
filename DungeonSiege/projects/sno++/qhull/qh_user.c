...
char qh_version[] = "GPG custom 98/7/25";  /* used for error messages */
...
{
  FILE *outfile= logfile;    /* output from qh_produce_output()
			       use NULL to skip qh_produce_output() */
  FILE *errfile= logfile;    /* error messages from qhull code */
  ...
  if (!exitcode) {                  /* if no error */
    FORALLfacets {
       /* ... your code ... */
	    qh_printafacet(logfile, qh_PRINTfacets, facet, 1);
    }
  }
  qh_freeqhull(!qh_ALL);
