...
/*HO */	DWORD          m_DataOffset;									// offset to start of data (used for RAW format)
		...
		//             ------ V1.0 Extra - Basic ------
/*12 */	MSQAVersion    m_ProductVersion;								// product version this tank was built with
/*12 */	MSQAVersion    m_MinimumVersion;								// minimum product version required to use this tank
...
/*ZST*/	WCHAR          m_AuthorText     [ AUTHOR_TEXT_LENGTH ];			// who made this tank
	...
	//                 and YY is the minor version.
		...
		bool ShouldOverride( const Header& other ) const;
	};
...
// Methods.
	...
	bool VerifyIndex( void );

	// access API
	File&           GetFile         ( void )                {  return ( m_TankFile );  }
	const File&     GetFile         ( void ) const          {  return ( m_TankFile );  }
