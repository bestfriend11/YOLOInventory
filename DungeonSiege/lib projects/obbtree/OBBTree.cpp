...
inline void ProjectVector(const vector_3& source_vec,
	...
	float test_val =  fabsf(InnerProduct(source_vec,onto_vec));		
...
int obb_disjoint(const matrix_3x3& B, const vector_3& T, const vector_3& a, const vector_3& b) {
  ...
  // Bf = fabs(B)
  Bf.SetElement_00( fabsf(B.GetElement_00()) + reps );
  Bf.SetElement_01( fabsf(B.GetElement_01()) + reps );
  Bf.SetElement_02( fabsf(B.GetElement_02()) + reps );
  Bf.SetElement_10( fabsf(B.GetElement_10()) + reps );
  Bf.SetElement_11( fabsf(B.GetElement_11()) + reps );
  Bf.SetElement_12( fabsf(B.GetElement_12()) + reps );
  Bf.SetElement_20( fabsf(B.GetElement_20()) + reps );
  Bf.SetElement_21( fabsf(B.GetElement_21()) + reps );
  Bf.SetElement_22( fabsf(B.GetElement_22()) + reps );
...
bool CheckForContact(const OBBNode& A_Node,const matrix_3x3& A_Orient, const vector_3& A_Trans,
	...
	if( drawboxes )
	{
		...
		DrawTheDamnLine(A,B,0xFFFF0000);
		DrawTheDamnLine(B,C,0xFFFF0000);
		DrawTheDamnLine(C,D,0xFFFF0000);
		DrawTheDamnLine(D,A,0xFFFF0000);

		DrawTheDamnLine(E,F,0xFFFFFFFF);
		DrawTheDamnLine(F,G,0xFFFFFFFF);
		DrawTheDamnLine(G,H,0xFFFFFFFF);
		DrawTheDamnLine(H,E,0xFFFFFFFF);

		DrawTheDamnLine(A,E,0xFFFFFFFF);
		DrawTheDamnLine(B,F,0xFFFFFFFF);
		DrawTheDamnLine(C,G,0xFFFFFFFF);
		DrawTheDamnLine(D,H,0xFFFFFFFF);
	...
	if( drawboxes )
	{
		...
		DrawTheDamnLine(A,B,0xFF0000FF);
		DrawTheDamnLine(B,C,0xFF0000FF);
		DrawTheDamnLine(C,D,0xFF0000FF);
		DrawTheDamnLine(D,A,0xFF0000FF);
