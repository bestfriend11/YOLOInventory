...
public:
	...
	// Typedefs
	typedef stdx::fast_vector< IntersectTestNode >	IntersectStack;
	typedef stdx::fast_vector< BSPNode* >			NodeStack;
	typedef stdx::fast_vector< unsigned short >		TriangleIndexColl;
	...
	bool		RayIntersectTreeTri( const vector_3& ray_orig, const vector_3& ray_dir,
									 float& ray_t, TriNorm& triangle );

	bool		YRayIntersectTree( const vector_3& ray_orig, const float ray_dir,
								   float& ray_t, vector_3& facenormal );
...
private:
	...
	void		DrawNode( Rapi& renderer, const BSPNode* node );
