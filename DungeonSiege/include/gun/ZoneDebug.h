...
extern BOOL __stdcall __AssertMsg( LPTSTR exp , LPTSTR file, int line );
...
//
	...
	#define ASSERT(exp) \
		do \
		{  \
			if ( !(exp) && __AssertMsg((LPTSTR)(#exp), (LPTSTR) __FILE__, __LINE__)) \
			{ \
				__asm int 0x03 \
			} \
		} while(0)
...
#define ERRORMSG(exp) \
		do \
		{  \
			if ( __AssertMsg((LPTSTR)(exp), (LPTSTR) __FILE__, __LINE__)) \
			{ \
				__asm int 0x03 \
			} \
		} while(0)
