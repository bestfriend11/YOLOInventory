...
bool UYIInventoryGameplaySetupLibrary::EnsureItemSupportsEquipSlot(UYIItemDefinition* ItemDef, FGameplayTag SlotTag, bool bSetItemTypeToSlotTag, bool bAddToOccupiedSlots)
{
	...
	{
		if (FInstancedStruct* EquipFragmentStruct = ItemDef->FindOrAddDefinitionFragmentByStruct(FYIItemEquipmentDefinitionFragment::StaticStruct()))
		{
			if (FYIItemEquipmentDefinitionFragment* EquipFragment = EquipFragmentStruct->GetMutablePtr<FYIItemEquipmentDefinitionFragment>())
			{
				if (!EquipFragment->OccupiedSlots.HasTagExact(SlotTag))
				{
					EquipFragment->OccupiedSlots.AddTag(SlotTag);
				}
			}
		}
	}
