...
static FProperty* FindPropertyByAuthoredNameAffixEditor(const UStruct* OwnerStruct, FName FieldName)
{
...
static UScriptStruct* ResolveStructFromPathStringAffixEditor(const FString& StructPath)
{
...
static void CollectFragmentStructOptionsAffixEditor(const UScriptStruct* BaseStruct, TArray<TSharedPtr<FString>>& OutOptions)
{
	OutOptions.Reset();
	if (!BaseStruct)
	{
...
static void CollectStructFieldOptionsAffixEditor(const UStruct* OwnerStruct, TArray<TSharedPtr<FString>>& OutOptions)
{
	OutOptions.Reset();
	if (!OwnerStruct)
	{
...
static bool HasMappingTargetV2Affix(const FYIFieldMapping& Mapping)
{
...
static bool ApplyInlineMappingsAffix(const UYIDataTableAffixSource* Source, const UDataTable* DataTable, FName RowName, UYIAffixAsset*& OutAffix)
{
	...
	for (const FYIFieldMapping& Mapping : Source->InlineMappings)
	{
		...
		if (!YIResolveMappingTarget(Affix, Mapping, ResolvedTarget, nullptr))
		{
			continue;
		}
		FProperty* TargetProp = ResolvedTarget.Property;
		uint8* DstPtr = ResolvedTarget.ValuePtr;
								...
								.Text_Lambda([this]()
									{
										...
										for (const FYIFieldMapping& M : CurrentSource->InlineMappings)
										{
											...
											const bool bHasTarget = HasMappingTargetV2Affix(M);
...
void SYIAffixDashboard::RefreshMappingPreview()
{
	...
	UYIAffixAsset* PreviewAffix = NewObject<UYIAffixAsset>();
	...
	for (const FYIFieldMapping& Mapping : Source->InlineMappings)
	{
		...
		if (PreviewAffix)
		{
			YIResolveMappingTarget(PreviewAffix, Mapping, ResolvedTarget, nullptr);
		...
		else if (Mapping.Conversion == EYIFieldMappingConversion::Vector2DFromXY)
		{
			...
			else
			{
				...
				else if (const FStructProperty* DestStruct = CastField<FStructProperty>(TargetProp))
				{
					...
					{
						FIntPoint* Point = reinterpret_cast<FIntPoint*>(Temp.GetData());
						*Point = FIntPoint((int32)X, (int32)Y);
						bConverted = true;
						Row->ConvertedValue = ExportPropertyValueToStringAffix(TargetProp, Temp.GetData());
					}
					else
...
void SYIAffixDashboard::AutoMatchInlineMappings(bool bAddAllFields)
{
	...
	for (FYIFieldMapping& M : Source->InlineMappings)
	{
		...
		if (M.bUseStaticValue || TargetField.IsNone() || !M.SourceField.IsNone())
		{
			continue;
		}
		const FName Match = FindBestMatch(TargetField);
		if (!Match.IsNone())
		{
			M.SourceField = Match;
			const FProperty* SourceProp = SourceFieldPropCache.FindRef(Match);
			const FProperty* TargetProp = nullptr;
			if (M.TargetLayer == EYIFieldMappingTargetLayer::LegacyProperty)
			{
				TargetProp = TargetFieldPropCache.FindRef(TargetField);
			}
			else if (const UScriptStruct* FragmentStruct = M.TargetFragmentStruct.Get())
			{
				TargetProp = FindPropertyByAuthoredNameAffixEditor(FragmentStruct, TargetField);
			}
...
TSharedRef<ITableRow> SYIAffixDashboard::MakeMappingRow(TSharedPtr<FYIFieldMapping> Mapping, const TSharedRef<STableViewBase>& OwnerTable)
{
	...
	auto GetTargetProp = [this, Mapping]() -> FProperty*
	{
		...
		return FindPropertyByAuthoredNameAffixEditor(FragmentStruct, YIGetResolvedTargetFieldName(*Mapping));
	};
	...
	{
		CollectFragmentStructOptionsAffixEditor(FYIAffixDefinitionFragmentBase::StaticStruct(), *TargetFragmentStructOptions);
	};
	...
	auto RefreshTargetFragmentFieldOptions = [Mapping, TargetFragmentFieldOptions]()
	{
		...
		CollectStructFieldOptionsAffixEditor(FragmentStruct, *TargetFragmentFieldOptions);
	};
	...
	auto PersistCurrentMapping = [this, Mapping]()
	{
		...
		if (Index != INDEX_NONE)
		{
			CurrentSource->InlineMappings[Index] = *Mapping;
		}
	};
						...
						SNew(SWidgetSwitcher)
							...
							[
								SNew(SCheckBox)
									...
									[
										SNew(STextBlock)
											.Text(NSLOCTEXT("YOLOInventory", "AffixDash_StaticBoolLabel", "Static Bool"))
									]
							]
							+ SWidgetSwitcher::Slot()
							[
										...
										SNew(STextBlock).Text_Lambda([Mapping]()
											{
												if (!Mapping.IsValid() || Mapping->StaticValue.IsEmpty())
												{
													return NSLOCTEXT("YOLOInventory", "AffixDash_StaticGameplayTagHint", "Select gameplay tag");
												}
												return FText::FromString(Mapping->StaticValue);
											})
									]
							...
							.OnSelectionChanged_Lambda([this, Mapping, GetSourceProp, GetTargetProp, RefreshTargetFragmentStructOptions, RefreshTargetFragmentFieldOptions, TargetFragmentStructOptions, TargetFragmentFieldOptions, PersistCurrentMapping, RefreshMappingUi](TSharedPtr<FString> NewItem, ESelectInfo::Type)
								{
									...
									else
									{
										if (!Mapping->TargetFragmentStruct)
										{
											...
											{
												Mapping->TargetFragmentStruct = ResolveStructFromPathStringAffixEditor(**(*TargetFragmentStructOptions)[0]);
											}
										}
									...
									if (Mapping->Conversion == EYIFieldMappingConversion::None)
									{
										const FProperty* SourceProp = GetSourceProp();
										const FProperty* TargetProp = GetTargetProp();
										Mapping->Conversion = GuessConversionForPropsAffix(SourceProp, TargetProp);
									}
											...
											.OnSelectionChanged_Lambda([this, Mapping, GetSourceProp, GetTargetProp, RefreshTargetFragmentFieldOptions, TargetFragmentFieldOptions, PersistCurrentMapping, RefreshMappingUi](TSharedPtr<FString> NewItem, ESelectInfo::Type)
												{
													...
													Mapping->TargetFragmentStruct = ResolveStructFromPathStringAffixEditor(**NewItem);
												...
												SNew(STextBlock).Text_Lambda([Mapping]()
													{
														...
														{
															return NSLOCTEXT("YOLOInventory", "AffixDash_TargetFragmentStructHint", "Select fragment");
														}
														...
														SNew(STextBlock).Text_Lambda([Mapping]()
															{
																...
																return ResolvedField.IsNone()
																	? NSLOCTEXT("YOLOInventory", "AffixDash_TargetFragmentFieldHint", "Select field")
