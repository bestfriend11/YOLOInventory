...
int32 UYIDungeonSiegeImporter::ImportTemplatesFromGas(const FYIDungeonSiegeImportOptions& Options)
{
	...
	UDataAssetFactory* DefFactory = Options.bCreateItemDefinitions ? NewObject<UDataAssetFactory>() : nullptr;
	if (DefFactory)
	{
		DefFactory->DataAssetClass = UYIItemDefinition::StaticClass();
	}
	UDataAssetFactory* LootFactory = Options.bCreateLootTable ? NewObject<UDataAssetFactory>() : nullptr;
	if (LootFactory)
	{
		LootFactory->DataAssetClass = UYILootTable::StaticClass();
	}
	UDataAssetFactory* RarityFactory = Options.bCreateRarityProfile ? NewObject<UDataAssetFactory>() : nullptr;
	if (RarityFactory)
	{
		RarityFactory->DataAssetClass = UYIRarityProfile::StaticClass();
	}
	UDataAssetFactory* GeneratorFactory = Options.bCreateItemGenerator ? NewObject<UDataAssetFactory>() : nullptr;
	if (GeneratorFactory)
	{
		GeneratorFactory->DataAssetClass = UYIItemGenerator::StaticClass();
	}
	...
	for (const FString& TemplateName : TemplateNames)
	{
		...
		if (Options.bCreateItemDefinitions && DefFactory)
		{
			...
			if (Def)
			{
				...
				if (FInstancedStruct* UIFragmentStruct = Def->FindOrAddDefinitionFragmentByStruct(FYIItemUIDefinitionFragment::StaticStruct()))
				{
					if (FYIItemUIDefinitionFragment* UIFragment = UIFragmentStruct->GetMutablePtr<FYIItemUIDefinitionFragment>())
					{
						UIFragment->DisplayName = FText::FromString(MakeDisplayName(TemplateName));
					}
				}
				Def->Modify();
				CreatedDefs.Add(Def);
